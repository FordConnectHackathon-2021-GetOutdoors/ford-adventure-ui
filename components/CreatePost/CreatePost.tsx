...
import { Box, Button, Center, Flex, Image, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, Spacer, Tag, TagCloseButton, TagLabel, useDisclosure, HStack } from "@chakra-ui/react";
...
export const CreatePost = forwardRef((props: FileProps, ref) => {
    ...
    const [currentLocationCoords, setCurrentLocationCoords] = useState({ lat: -1, lng: -1 });
  ...
  const handlePostContentChange = (e: any) => {
    ...
    const submitPost = () => {
        uploadImage(fileName, ext, imageBase64, content, currentLocationCoords.lat, currentLocationCoords.lng);
        onClose();
    };
                                        ...
                                        onClick={() => {
                                            ...
                                            if (location) {
                                                location.getCurrentPosition(async (position) => {
                                                    setCurrentLocationCoords({ lat: position.coords.latitude, lng: position.coords.longitude });
                                                    const shortLocation = await getShortLocationPromise(position.coords.latitude, position.coords.longitude);
                                    ...
                                    />
                                    {
                                        ...
                                        <Center>
                                            <Tag>
                                                ...
                                                <TagCloseButton onClick={() => setCurrentLocation("")} />
                                            </Tag>
                                        </Center>
                                    }
                                </HStack>
