...
export function PhotoFeedProvider({ children }) {
  ...
  const fetchFeedData = async (tag) => {
    ...
    const post_user_ids: string[] = Array.from(
      new Set([...user_posts.map(({ user_id }) => user_id)])
    );
    ...
    const _user_posts = user_posts
      ...
      .map(({ image_base64, created, content, user_id, likes, comments }) => {
        ...
        return {
          ...
          profile_pic_base64,
          user_id,
          authUserUuid: authUser ? authUser["id"] : 0,
          likes,
          comments,
        };
      });
